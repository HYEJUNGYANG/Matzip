...
function SearchRegionResult({regionInfo}: SearchRegionResultProps) {
  ...
  const getUnitOfDistance = (distance: number): string => {
    const changeDistance = distance / 1000;

    if (changeDistance >= 1) {
      return `${changeDistance.toFixed(2)}km`;
    }
    return `${distance}m`;
  };
          ...
          <Pressable
            ...
            <View style={styles.categoryContainer}>
              <Text style={styles.distanceText}>
                {getUnitOfDistance(Number(info.distance))}
              </Text>
